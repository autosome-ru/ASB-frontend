...
This release includes allele-specific chromatin accessibility sites (ASAs) obtained by meta-analysis of DNase-/ATAC-/FAIRE-Seq read alignments of GTRD v. 21.12.

Release files structure:
Each experiment type folder contains multiple separate files listing all putative ASA events in a particular cell type at eligible SNVs that pass the necessary coverage thresholds.
...
Each tsv-file is a plain tab-separated text document containing one line per single-nucleotide variant (SNV) with the following columns.
  ...
  'n_reps: number of aggregated observations (datasets) for a particular ASA
